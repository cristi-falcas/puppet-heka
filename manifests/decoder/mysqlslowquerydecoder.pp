# Parses and transforms the MySQL slow query logs. Use mariadb_slow_query.lua to parse the MariaDB variant of the MySQL slow query logs.
# It uses the sandboxdecoder
#
# === Parameters:
#
# $ensure::                       This is used to set the status of the config file: present or absent
#                                 Default: present
#
### Check heka::decoder::sandboxdecoder for common parameters
#
### MySQL Slow Query Log Parameters
#
# $truncate_sql::                 Truncates the SQL payload to the specified number of bytes (not UTF-8 aware) and appends "...".
#                                 If the value is nil no truncation is performed. A negative value will truncate the specified
#                                 number of bytes from the end.
#                                 Default to nil
#                                 Type: string
#
define heka::decoder::mysqlslowquerydecoder (
  $ensure            = 'present',
  # Common Sandbox Parameters
  $preserve_data     = undef,
  $memory_limit      = undef,
  $instruction_limit = undef,
  $output_limit      = undef,
  $module_directory  = undef,
  # MySQL Slow Query Parameters
  $truncate_sql      = undef,
) {
  validate_re($ensure, '^(present|absent)$')
  # Common Sandbox Parameters
  if $preserve_data { validate_bool($preserve_data) }
  if $memory_limit { validate_integer($memory_limit) }
  if $instruction_limit { validate_integer($instruction_limit) }
  if $output_limit { validate_integer($output_limit) }
  if $module_directory { validate_string($module_directory) }
  # MySQL Slow Query Parameters
  if $truncate_sql { validate_integer($truncate_sql) }

  $sandbox_type = 'SandboxDecoder'
  $script_type = 'lua'
  $filename = 'lua_decoders/mysql_slow_query.lua'
  $config = {
    'truncate_sql' => $truncate_sql,
  }

  $full_name = "mysqlslowquerydecoder_${name}"
  heka::snippet { $full_name:
    ensure  => $ensure,
    content => template("${module_name}/sandbox.toml.erb"),
  }
}
