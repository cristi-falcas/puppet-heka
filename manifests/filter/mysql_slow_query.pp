...
#
### MySQL Slow Query Parameters
#
# $sec_per_row::                  Sets the size of each bucket (resolution in seconds) in the sliding window.
#                                 Default 60
#                                 Type: uint
#
# $rows::                         Sets the size of the sliding window i.e., 1440 rows representing 60 seconds per row is a 24
#                                 sliding hour window with 1 minute resolution.
#                                 Default 1440
#                                 Type: uint
#
# $preservation_version::         If preserve_data = true is set in the SandboxFilter configuration, then this value should be
#                                 incremented every time the sec_per_row or rows configuration is changed to prevent the plugin
#                                 from failing to start during data restoration.
#                                 Default 0
#                                 Type: uint
#
define heka::filter::mysql_slow_query (
  $ensure               = 'present',
  ...
  # MySQL Slow Query Parameters
  validate_integer($sec_per_row, $rows)
  ...
  $full_name = "mysql_slow_query_${name}"
